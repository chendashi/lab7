...
double distanceBetween(Point p, Point q) {
  double temp = sqrt(pow(p.x - q.x, 2.0) + pow(p.y - q.y, 2.0));
...
bool boxesApproxEqual(Box b1, Box b2, double tolerance) {
  if(fabs(b1.ul.x - b2.ul.x) > tolerance || fabs(b1.ul.y - b2.ul.y) > tolerance || fabs(b1.width  - b2.width) > tolerance || fabs(b1.height  - b2.height) > tolerance){
      return false;
  }
  return true;
